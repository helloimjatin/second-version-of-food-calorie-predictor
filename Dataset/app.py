...
# (Joblib needs this class definition to exist in the namespace)
...
def find_database_match(user_input, db):
    ...
    matches = difflib.get_close_matches(user_input.lower(), db['Dish_Name'].astype(str), n=1, cutoff=0.85)
    if matches:
        match_name = matches[0]
        row = db[db['Dish_Name'] == match_name].iloc[0]
        return row
    return None
